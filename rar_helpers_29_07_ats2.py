...
def find_freqs(system, driver_freq):
                ...
                for l in range(4):
                    for m in range(4):
                        for n in range(4):
                            ...
                            b_pow = (k + n)
                            c_pow = (j + m)
                            parity = (a_pow + b_pow + c_pow) % 2
                            if parity == 1:
                                key = f"a: {i} - b: {k} - c: {j} to a: {l} - b: {n} - c: {m}"
                                freq = Es[i, j, k] - Es[l, m, n] - driver_freq
                                freqs[key] = jnp.array(freq, float)

    freqs_ordered = dict(sorted(freqs.items(), key=lambda item: abs(item[1])))
    return freqs_ordered
...
def create_system_linear_rar(params):
    ...
    g_ac = params.get("g_ac", J_ac * resonator_a.n_zpf() * ats.n_zpf())
    g_cb = params.get("g_cb", J_cb * ats.n_zpf() * resonator_b.n_zpf())
...
def get_metrics_linear_rar(params):
    ...
    for j, d1 in enumerate(system.devices):
        phi_0 = system.promote(d1.ops["phi"], j)
        for d2 in system.devices:
            ϕ[d1.label][d2.label] = (jqt.dag(e[d2.label]) @ phi_0 @ vac).data[0][0]
